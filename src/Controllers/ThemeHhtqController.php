...
namespace Ophim\ThemeHhtq\Controllers;
...
class ThemeHhtqController
{
    public function index(Request $request)
    {
        if ($request['search'] || $request['filter']) {
            ...
            return view('themes::themehhtq.catalog', [
                'data' => $data,
                'search' => $request['search'],
                'section_name' => "Tìm kiếm phim: $request->search"
            ]);
        }
        return view('themes::themehhtq.index', [
    ...
    public function getMovieOverview(Request $request, $movie)
    {
        ...
        $tops_cache_key = 'site.movies.tops.' . $movie->type;
        $tops = Cache::get($tops_cache_key);
        if(is_null($tops)) {
            $tops = Movie::where('type', $movie->type)
            ->orderBy('view_week', 'desc')
            ->limit(8)
            ->get();
            Cache::put($tops_cache_key, $tops, setting('site_cache_ttl', 5 * 60));
        }
        ...
        return view('themes::themehhtq.single', [
            'currentMovie' => $movie,
    ...
    public function getEpisode(Request $request, $movie, $slug, $id)
    {
        ...
        return view('themes::themehhtq.episode', [
            'currentMovie' => $movie,
            'movie_related' => $movie_related,
            'episode' => $episode,
